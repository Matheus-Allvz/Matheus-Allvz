# Matheus Alves da Costa

<p align="center">
  <!-- Typing SVG Dynamic Header -->
  <a href="https://matheus-alves.dev">
    <img src="https://readme-typing-svg.herokuapp.com?font=Syne&weight=700&size=24&duration=3000&pause=1000&color=D63A2F&center=true&vCenter=true&width=820&height=50&lines=Backend+Software+Engineer+%2F%2F+C%23+.NET+8;Clean+Architecture+%7C+CQRS+%7C+Distributed+Systems;High-Throughput+Pipelines+%7C+RabbitMQ+%7C+PostgreSQL;B.S.+in+Computer+Science+%40+PUC+Goi%C3%A1s+(CR+8.6)" alt="Typing SVG Header" />
  </a>
</p>

<p align="center">
  <strong>🚀 Desenvolvedor Backend C# (.NET 8) na Actuar</strong> &bull; 📍 Goiânia, GO &bull; 🎓 <strong>Ciência da Computação (PUC-GO)</strong>
</p>

<p align="center">
  <a href="https://matheus-alves.dev"><img src="https://img.shields.io/badge/Portf%C3%B3lio_Editorial-D63A2F?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio"/></a>
  <a href="https://dataclean.matheus-alves.dev"><img src="https://img.shields.io/badge/DataClean_Live-00F2FE?style=for-the-badge&logo=c-sharp&logoColor=black" alt="DataClean Web"/></a>
  <a href="https://www.linkedin.com/in/matheusallvz"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
  <a href="mailto:workingaccount.matheus@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
  <a href="https://wa.me/5562993123343"><img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="WhatsApp"/></a>
</p>

```text
┌─────────────────────────┬─────────────────────────────┬─────────────────────────────┐
│  🏆 5x TOP 1 ANALYST    │  🛡️ 14.75 / 15.0 RCA CERT   │  ⚡ 99.4% SPEED GAIN        │
│  Actuar Ranking Geral   │  Arquitetura & Domínio      │  DataClean C# .NET 8 Engine │
└─────────────────────────┴─────────────────────────────┴─────────────────────────────┘
```

---

### 👨‍💻 Sobre Mim & Trajetória Técnica

Engenheiro de Software com foco em **C# 12 / .NET 8**, arquiteturas distribuídas de **alto throughput**, mensageria assíncrona com **RabbitMQ**, modelagem de dados relacional (**PostgreSQL / SQL Server**) e resiliência de microsserviços.

Na **Actuar**, construí uma evolução técnica consistente e acelerada:
* 🏆 **Analista N1**: Eleito **5 vezes Analista Top 1** e **1 vez Top 2** no ranking geral de performance técnica e resolução em 8 meses.
* 🛡️ **Analista N2**: Ponto focal de investigação e diagnóstico em código (**Root Cause Analysis**) para incidentes de alta complexidade em produção. Aprovado com **nota 14.75 / 15.0** na certificação técnica de arquitetura de regras de negócio.
* 🚀 **Desenvolvedor C# Jr**: Atuação direta na sustentação e evolução de microsserviços, pipelines de higienização massiva de dados e engenharia de automação.

---

### 🏛️ Arquitetura de Referência & Padrões

```text
 [ HTTP Client / UI ] ──────────► [ ASP.NET Core 8 Web API ]
                                            │
               ┌────────────────────────────┴────────────────────────────┐
               ▼                                                         ▼
    [ Command Flow (CQRS) ]                                   [ Query Flow (Dapper) ]
    ├── MediatR Command Handlers                              ├── Optimized SQL / CTEs
    ├── FluentValidation Pipeline Behaviors                   ├── B-Tree Index Lookups
    └── Domain Events & Outbox Pattern                        └── Read Models & Projections
               │                                                         │
               ├────────────────────────────┬────────────────────────────┤
               ▼                            ▼                            ▼
    [ PostgreSQL 16 (ACID) ]      [ RabbitMQ / MassTransit ]    [ Distributed Cache ]
```

---

### 🛠️ Tecnologias & Engenharia

<p align="center">
  <!-- Backend & Architecture -->
  <img src="https://img.shields.io/badge/C%23_12-239120?style=flat-square&logo=c-sharp&logoColor=white" alt="C#"/>
  <img src="https://img.shields.io/badge/.NET_8-512BD4?style=flat-square&logo=dotnet&logoColor=white" alt=".NET 8"/>
  <img src="https://img.shields.io/badge/ASP.NET_Core_WebAPI-512BD4?style=flat-square&logo=dotnet&logoColor=white" alt="ASP.NET Core"/>
  <img src="https://img.shields.io/badge/Clean_Architecture-00F2FE?style=flat-square&logo=blueprint&logoColor=black" alt="Clean Architecture"/>
  <img src="https://img.shields.io/badge/CQRS_%26_DDD-8B5CF6?style=flat-square&logo=blueprint&logoColor=white" alt="CQRS"/>
  <img src="https://img.shields.io/badge/PostgreSQL_16-4169E1?style=flat-square&logo=postgresql&logoColor=white" alt="PostgreSQL"/>
  <img src="https://img.shields.io/badge/Entity_Framework_Core-512BD4?style=flat-square&logo=dotnet&logoColor=white" alt="EF Core"/>
  <img src="https://img.shields.io/badge/Dapper-00599C?style=flat-square&logo=cplusplus&logoColor=white" alt="Dapper"/>
  <img src="https://img.shields.io/badge/RabbitMQ-FF6600?style=flat-square&logo=rabbitmq&logoColor=white" alt="RabbitMQ"/>
  <img src="https://img.shields.io/badge/Docker_Compose-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Docker"/>
  <img src="https://img.shields.io/badge/xUnit_Testing-1572B6?style=flat-square&logo=testinglibrary&logoColor=white" alt="xUnit"/>
</p>

<p align="center">
  <!-- Frontend, Scripting & Hardware -->
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript"/>
  <img src="https://img.shields.io/badge/React_%2F_Vite-61DAFB?style=flat-square&logo=react&logoColor=black" alt="React"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python"/>
  <img src="https://img.shields.io/badge/Linguagem_C-00599C?style=flat-square&logo=c&logoColor=white" alt="C"/>
  <img src="https://img.shields.io/badge/RP2040_%2F_IoT-00979D?style=flat-square&logo=arduino&logoColor=white" alt="RP2040 IoT"/>
  <img src="https://img.shields.io/badge/Git_%26_GitHub-F05032?style=flat-square&logo=git&logoColor=white" alt="Git"/>
  <img src="https://img.shields.io/badge/Linux_%2F_Bash-FCC624?style=flat-square&logo=linux&logoColor=black" alt="Linux"/>
</p>

---

### 🌟 Repertório de Engenharia & Principais Sistemas

#### 1. 🏢 [DataClean Microservice & Web Suite](https://dataclean.matheus-alves.dev)
> **Engine C# .NET 8 em Produção para Ingestão, Normalização e Higienização Massiva de Dados**
* **Aplicação em Produção**: [dataclean.matheus-alves.dev](https://dataclean.matheus-alves.dev)
* **Benchmark de Performance**: Redução de tempo de processamento de lotes de **6min02s para 1.84s (99.4% de ganho)**.
* **Clean Architecture**: Segregação estrita entre *Domain*, *Application*, *Infrastructure* e *API*.
* **Multi-Format Excel Parser**: Engine para XLSX, XLS (OLE2 Compound Document), SpreadsheetML 2003 (XML), CSV com delimitador heurístico e ODS.
* **Algoritmo de Scoring de Cabeçalhos**: Detecção probabilística de padrões em 25 linhas avaliando densidade de texto, unicidade e regex restritivas (CPF, CNPJ, Datas, Telefones).
* **Stack**: C# 12 / .NET 8, PostgreSQL 16, RabbitMQ, Docker Multi-Stage, React + TypeScript.

#### 2. ⚡ [Meppo — Productivity & Automation Suite](https://www.linkedin.com/posts/matheusallvz_produtividade-customerexperience-desenvolvimento-activity-7432189024691404802-HZ0r)
> **Suíte de Automação Operacional & Orquestração de APIs**
* Otimizou procedimentos operacionais de **6 minutos para poucos segundos** sem degradação de consistência.
* Orquestração assíncrona de APIs REST, unificação de busca dinâmica e redução drástica de sobrecarga cognitiva.

#### 3. 🛡️ [RCA Telemetry & Diagnostic Framework](https://www.linkedin.com/in/matheusallvz)
> **Framework de Diagnóstico e Rastreamento de Causa Raiz**
* Rastreabilidade e análise de causa raiz em microsserviços distribuídos, isolamento de condições de corrida (*race conditions*) e garantia de integridade transacional ACID.

#### 4. 🧠 [Simuladores de Hardware & Arquitetura de Computadores](https://github.com/Matheus-Allvz/Cache-Simulator)
* **[Cache-Simulator](https://matheus-allvz.github.io/Cache-Simulator/)**: Simulador interativo da hierarquia de memória cache (L1, L2, L3) e CPU para análise de *hits/misses* e latência.
* **[Pipeline-Simulator](https://github.com/Matheus-Allvz/Pipeline-Simulator)**: Simulador visual de instruções em pipeline de CPU (Fetch, Decode, Execute, Memory, Writeback) e tratamento de hazards estruturais e de dados.

---

### 📊 Telemetria de Repositórios & Commits

<p align="center">
  <img src="./github-metrics/isocalendar.svg" alt="Isometric Commit Calendar" width="100%"/>
</p>

<p align="center">
  <img src="./github-metrics/traffic.svg" alt="Repositories & Languages Matrix" width="100%"/>
</p>

---

### 🎓 Formação Acadêmica & Certificações

* 🏛️ **Bacharelado em Ciência da Computação** — Pontifícia Universidade Católica de Goiás (PUC-GO) | *2024 — 2029* (CR: **8.6**)
  * *Foco em Algoritmos, Estruturas de Dados, Sistemas Distribuídos, Sistemas Operacionais e Engenharia de Software.*
* 📜 **Certificação Interna Actuar**: Regras de Negócio e Arquitetura de Domínio — Nota **14.75 / 15.0**.
* 🌐 **Idiomas**: Português (Nativo), Inglês Técnico Avançado (Documentação, RFCs e conversação).

---

### 📬 Contato & Canais Diretos

* 🌐 **Portfólio Oficial**: [matheus-alves.dev](https://matheus-alves.dev)
* 💼 **LinkedIn**: [linkedin.com/in/matheusallvz](https://www.linkedin.com/in/matheusallvz)
* 📧 **E-mail**: [workingaccount.matheus@gmail.com](mailto:workingaccount.matheus@gmail.com)
* 💬 **WhatsApp**: [+55 (62) 99312-3343](https://wa.me/5562993123343)
